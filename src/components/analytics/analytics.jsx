...
import { SkeletonCards, SkeletonBlock } from '../layout/Skeleton'
...
export default function Analytics() {
  ...
  if (loading) return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ marginBottom: 28 }}>
        <div style={{ height: 28, width: 180, background: COLORS.surfaceAlt, borderRadius: 8, marginBottom: 8, animation: "shimmer 1.5s infinite" }} />
        <div style={{ height: 14, width: 260, background: COLORS.surfaceAlt, borderRadius: 6, animation: "shimmer 1.5s infinite" }} />
      </div>
  ...
  const deptData = departments.map(dept => {
    ...
    const rate = deptRecords.length > 0
      ? Math.round((present / deptRecords.length) * 100) : 0
    return { dept, rate, count: employees.filter(e => e.department === dept).length }
  })
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: "-0.02em" }}>Analytics</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        {[
          { label: isManager ? "Attendance Rate"  : "My Attendance Rate", value: `${attendanceRate}%`,         sub: `${presentRecords} of ${totalRecords} records`,          color: COLORS.green, icon: "✓" },
          { label: isManager ? "Absenteeism Rate" : "My Absenteeism",     value: `${absenteeismRate}%`,        sub: `${absentRecords} absent records`,                       color: COLORS.red,   icon: "✗" },
          { label: isManager ? "Total Hours"      : "My Total Hours",     value: `${totalHours.toFixed(0)}h`,  sub: isManager ? "across all employees" : "this period",      color: COLORS.accent, icon: "⏱" },
          { label: isManager ? "Total Overtime"   : "My Overtime",        value: `${totalOvertime.toFixed(1)}h`, sub: isManager ? "across all employees" : "this period",    color: COLORS.amber, icon: "⚡" },
        ].map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: 28, fontWeight: 800, color: m.color, fontFamily: "'DM Mono', monospace", letterSpacing: "-0.02em" }}>
              {m.value}
            </div>
            <div style={{ color: COLORS.text, fontSize: 13, fontWeight: 600, marginTop: 4 }}>{m.label}</div>
            <div style={{ color: COLORS.textMuted, fontSize: 11, marginTop: 2 }}>{m.sub}</div>
          </div>
        ))}
      </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 16, marginBottom: 16 }}>
            ...
            <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 24 }}>
              ...
              <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
                {[{ label: "Present", color: COLORS.accent }, { label: "Absent", color: COLORS.red }].map(l => (
                ...
                {chartData.map((d, i) => (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, height: "100%", justifyContent: "flex-end" }}>
                    <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 2, justifyContent: "flex-end" }}>
                      {d.absent > 0 && (
                        <div style={{ width: "100%", height: `${(d.absent / maxBarValue) * 120}px`, background: COLORS.red, borderRadius: "4px 4px 0 0", opacity: 0.7, minHeight: 4 }} />
                      )}
                      {d.present > 0 && (
                        <div style={{ width: "100%", height: `${(d.present / maxBarValue) * 120}px`, background: COLORS.accent, borderRadius: d.absent > 0 ? 0 : "4px 4px 0 0", minHeight: 4 }} />
                      )}
                      {d.present === 0 && d.absent === 0 && (
                        <div style={{ width: "100%", height: 4, background: COLORS.border, borderRadius: 4 }} />
                      )}
                    </div>
                    <div style={{ color: COLORS.textMuted, fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{d.label}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {deptData.map((d, i) => (
                  <div key={d.dept}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 6 }}>
                      ...
                      <span style={{ color: deptColors[i % deptColors.length], fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>
                        {d.rate}%
                      </span>
                    </div>
                    <div style={{ background: COLORS.surfaceAlt, borderRadius: 4, height: 8, overflow: "hidden" }}>
                      <div style={{ width: `${d.rate}%`, height: "100%", background: deptColors[i % deptColors.length], borderRadius: 4, transition: "width 0.6s ease" }} />
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {overtimeByEmployee.map((emp, i) => (
                <div key={emp.name} style={{
                  ...
                  <div style={{ fontSize: 11, color: i === 0 ? COLORS.amber : COLORS.textMuted, fontFamily: "'DM Mono', monospace", fontWeight: 700, marginBottom: 8 }}>
                    #{i + 1}
                  </div>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(135deg, ${COLORS.accent}33, ${COLORS.purple}33)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: COLORS.accent, margin: "0 auto 10px", fontFamily: "'DM Mono', monospace" }}>
                    {emp.avatar}
                  </div>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2 }}>{emp.name.split(' ')[0]}</div>
                  <div style={{ color: COLORS.textMuted, fontSize: 11, marginBottom: 8 }}>{emp.department}</div>
                  <div style={{ color: COLORS.red, fontFamily: "'DM Mono', monospace", fontWeight: 800, fontSize: 16 }}>
        ...
        <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {myWeekRecords.map((d, i) => (
                <div key={i} style={{
                  ...
                  padding: "10px 14px", background: COLORS.surfaceAlt, borderRadius: 10,
                  borderLeft: `3px solid ${statusColor[d.status] || COLORS.border}`,
                }}>
                  <div style={{ color: COLORS.textMuted, fontSize: 12, fontFamily: "'DM Mono', monospace", width: 32, fontWeight: 700 }}>
                    {d.label}
                  </div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: statusColor[d.status] || COLORS.textMuted, textTransform: "uppercase", letterSpacing: "0.06em", width: 90 }}>
                    {d.status.replace('-', ' ')}
                  </div>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, color: COLORS.textMuted, flex: 1 }}>
                    {d.clockIn !== '—' ? `${d.clockIn} → ${d.clockOut}` : '—'}
                  </div>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, color: COLORS.accent, width: 40, textAlign: "right" }}>
                    {d.hours > 0 ? `${d.hours}h` : '—'}
                  </div>
                  {d.overtime > 0 && (
                    <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: COLORS.red, fontWeight: 700 }}>
                      +{d.overtime}h OT
                    </div>
                  )}
                </div>
          ...
          <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {[
                { label: "Days Present",  value: presentRecords,              color: COLORS.green,  sub: `of ${totalRecords} days`  },
                { label: "Hours Worked",  value: `${totalHours.toFixed(1)}h`, color: COLORS.accent, sub: "this period"              },
                { label: "Overtime",      value: `${totalOvertime.toFixed(1)}h`, color: COLORS.red, sub: "this period"              },
              ].map((s, i) => (
                <div key={i} style={{ background: COLORS.surfaceAlt, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 20, textAlign: "center" }}>
                  ...
                  <div style={{ color: COLORS.text, fontSize: 13, fontWeight: 600, marginTop: 6 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
