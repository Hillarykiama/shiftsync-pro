...
import { SkeletonTable, SkeletonCards } from '../layout/Skeleton'
...
export default function Attendance() {
  ...
  const rows = filteredEmployees.map(emp => {
    ...
    return {
      ...
      clockIn:    att?.clock_in    ? new Date(att.clock_in).toLocaleTimeString([],  { hour: '2-digit', minute: '2-digit' }) : null,
      clockOut:   att?.clock_out   ? new Date(att.clock_out).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : null,
  ...
  if (loading) return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ height: 14, width: 320, background: COLORS.surfaceAlt, borderRadius: 6, animation: "shimmer 1.5s infinite" }} />
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            <div key={i} style={{
              ...
              <div style={{ fontSize: 32, fontWeight: 800, color: s.color, fontFamily: "'DM Mono', monospace" }}>{s.count}</div>
              <div style={{ color: s.color, fontSize: 14, fontWeight: 600 }}>{s.label}</div>
            </div>
          ))}
        </div>
      )}

      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 24, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${COLORS.border}` }}>
              {["Employee", "Department", "Shift", "Clock In", "Clock Out", "Hours", "Status", "Overtime", ...(isManager ? ["IP Address"] : [])].map(h => (
                <th key={h} style={{
          ...
          <tbody>
            {rows.map((emp, i) => (
              ...
              >
                ...
                <td style={{ padding: "14px 12px", color: COLORS.textMuted, fontSize: 13 }}>{emp.department}</td>
                <td style={{ padding: "14px 12px", fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{emp.shift}</td>
                <td style={{ padding: "14px 12px", fontFamily: "'DM Mono', monospace", fontSize: 13, color: emp.clockIn ? COLORS.green : COLORS.textDim }}>
                  {emp.clockIn || '—'}
                </td>
                <td style={{ padding: "14px 12px", fontFamily: "'DM Mono', monospace", fontSize: 13, color: emp.clockOut ? COLORS.textMuted : COLORS.textDim }}>
                  {emp.clockOut || '—'}
                </td>
                ...
                {isManager && (
                  <td style={{ padding: "14px 12px", fontFamily: "'DM Mono', monospace", fontSize: 11, color: COLORS.textMuted }}>
                    {emp.ipAddress}
                  </td>
                )}
              </tr>
