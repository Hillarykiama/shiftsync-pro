  ...
  if (isMobile) {
    return (
      <>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              ...
              style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "6px 10px", color: COLORS.textMuted, cursor: "pointer", fontSize: 12, fontFamily: "'DM Mono', monospace" }}
            >
              📸
            </button>
        ...
        <div style={{
          ...
          {visibleItems.slice(0, 5).map(n => (
            ...
            >
              ...
              <span style={{ fontSize: 9, fontWeight: currentView === n.id ? 700 : 400, textTransform: "uppercase", letterSpacing: "0.04em", fontFamily: "'DM Mono', monospace" }}>
                {n.label}
              </span>
            </button>
          ))}
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                style={{
                  ...
                  display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
                  cursor: "pointer", padding: "6px 8px", borderRadius: 10,
                  color: visibleItems.slice(5).some(n => n.id === currentView) ? COLORS.accent : COLORS.textMuted,
                }}
              >
                <span style={{ fontSize: 18 }}>⋯</span>
                <span style={{ fontSize: 9, fontWeight: 400, textTransform: "uppercase", letterSpacing: "0.04em", fontFamily: "'DM Mono', monospace" }}>
                  More
                </span>
              </button>
              ...
              {showMore && (
                <div style={{
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", borderRadius: 8, padding: "10px 14px", color: COLORS.textMuted, cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontSize: 13, width: "100%", fontFamily: "'Sora', sans-serif" }}
                  >
                    📸 Register Face
                  </button>
                </div>
        ...
        {showFaceReg && (
          <FaceRegistration onClose={() => setShowFaceReg(false)} onSuccess={() => setShowFaceReg(false)} />
        )}
      </>
    )
  }
    ...
    <>
      <div style={{
        ...
        <div style={{ marginBottom: 32, padding: "0 8px" }}>
          <div style={{ color: COLORS.accent, fontSize: 20, fontWeight: 800, letterSpacing: "-0.02em" }}>ShiftSync</div>
        ...
        {/* User Profile */}
        <div style={{ background: COLORS.surfaceAlt, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <Avatar initials={user?.avatar || '..'} size={32} />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{user?.name || 'Loading...'}</div>
            <div style={{ fontSize: 11, color: COLORS.textMuted }}>{isManager ? '⭐ Manager' : 'Employee'}</div>
          </div>
        </div>
        ...
        <button
          onClick={() => setShowFaceReg(true)}
          style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "10px 14px", color: COLORS.textMuted, cursor: "pointer", fontSize: 13, width: "100%", marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all 0.15s" }}
          onMouseEnter={e => { e.currentTarget.style.background = COLORS.accentGlow; e.currentTarget.style.color = COLORS.accent; e.currentTarget.style.borderColor = `${COLORS.accent}44` }}
