...
import { getEmployees, getTodayAttendance, resetEmployeeFace } from '../../lib/db'
  ...
  if (loading) return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ height: 14, width: 240, background: COLORS.surfaceAlt, borderRadius: 6, animation: "shimmer 1.5s infinite" }} />
      </div>
    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ color: COLORS.textMuted, fontSize: 14, marginTop: 4 }}>
          {isManager ? 'All employees · Live status' : 'Your profile and attendance summary'}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: COLORS.textMuted, fontSize: 12, marginTop: 2 }}>{emp.role}</div>
                    <div style={{ color: COLORS.textDim, fontSize: 11, marginTop: 2, fontFamily: "'DM Mono', monospace" }}>
                      {emp.department}
                    </div>
                  </div>
                </div>
              ...
              <div style={{
                ...
                <span style={{ fontFamily: "'DM Mono', monospace", color: COLORS.text, fontWeight: 600 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
                {[
                  ...
                  { label: "Face ID",   value: emp.face_registered ? '✓ Set' : '✗ Not set',                      color: emp.face_registered ? COLORS.green : COLORS.red },
                ].map((s, i) => (
                  <div key={i} style={{ background: COLORS.surfaceAlt, borderRadius: 8, padding: "8px 10px" }}>
                    ...
                    <div style={{ color: s.color, fontFamily: "'DM Mono', monospace", fontWeight: 700, fontSize: 13, marginTop: 2 }}>
                      {s.value}
                    </div>
                  </div>
                ))}
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: COLORS.textMuted, marginBottom: 5 }}>
                  ...
                  <span style={{ fontFamily: "'DM Mono', monospace" }}>{hoursToday}h / 8h</span>
                </div>
              ...
              {/* Reset Face — managers only */}
              {isManager && (
                <button
                  onClick={async () => {
                    if (window.confirm(`Reset face data for ${emp.name}? They will need to re-register before clocking in.`)) {
                      await resetEmployeeFace(emp.id)
                      load()
                    }
                  }}
                  style={{
